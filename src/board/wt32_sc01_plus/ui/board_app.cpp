...
#include "safety.h"
#include "core/build_info.h"   // FW_VERSION (regenerated by inject_version.py)
...
// Recursive mutex protecting LVGL state. loopTask takes it around
// lv_timer_handler (via BoardApp::loop); any AsyncTCP-task code that
// touches lv_* APIs (screenshot endpoint, future cross-task widget
// updates) MUST take it via BoardApp::lvLock / lvUnlock first.
// LVGL itself is configured with LV_USE_OS=LV_OS_NONE so its built-in
// lv_lock is a no-op -- this is our equivalent.
static SemaphoreHandle_t s_lv_mutex = nullptr;

// Lock with bounded wait. portMAX_DELAY here would risk tripping the
// TWDT on the AsyncTCP task if loopTask is doing something slow under
// the lock. 500 ms is generous (one LVGL frame is ~16 ms; even a heavy
// snapshot is well under 100 ms) but short enough that a stuck loopTask
// fails the lock attempt instead of taking the AsyncTCP task down with it.
bool BoardApp::lvLock() {
    if (!s_lv_mutex) return true;   // pre-begin: no-op success
    return xSemaphoreTakeRecursive(s_lv_mutex, pdMS_TO_TICKS(500)) == pdTRUE;
}
void BoardApp::lvUnlock() {
...
static lv_obj_t   *s_status_version = nullptr;   // pinned to right edge, fw tag
...
static void buildStatusBar(lv_obj_t *parent) {
    ...
    // Spacer pushes version + busy pill to the right edge.
    lv_obj_t *spacer = lv_obj_create(s_status_bar);
    lv_obj_remove_style_all(spacer);
    lv_obj_set_flex_grow(spacer, 1);
    lv_obj_set_size(spacer, 0, 1);

    // Firmware version -- right-aligned identity. Comes from
    // src/core/build_info.h, regenerated by scripts/inject_version.py
    // from `git describe --tags --exact-match` or "dev-<sha>" otherwise.
    s_status_version = lv_label_create(s_status_bar);
    lv_obj_set_style_text_color(s_status_version, lv_color_hex(0x6c7480), 0);
    lv_obj_set_style_text_font(s_status_version, &lv_font_montserrat_14, 0);
    lv_label_set_text(s_status_version, FW_VERSION);
    lv_obj_set_style_pad_right(s_status_version, 8, 0);
...
void BoardApp::begin(BoardDisplay &display) {
    ...
    // Mutex must exist before lv_init() so any early registrations
    // (display init callbacks, indev setup) fall under it cleanly.
...
void BoardApp::loop() {
    // Take lvLock around lv_timer_handler so AsyncTCP-task code that
    // grabs the same mutex (e.g. /api/sys/screenshot.bmp) can't race
    // widget mutations / animations / deletes with us.
    if (lvLock()) {
        lv_timer_handler();
        lvUnlock();
    }
